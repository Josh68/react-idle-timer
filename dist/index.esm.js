...
function withIdleTimer(Component2) {
  return forwardRef(/* @__PURE__ */ __name(function IdleTimer(props, ref) {
    const options = {...props};
...
var IIdleTimerComponent = class extends Component {
};
...
function useIdleTimer({
  ...
  const promptTimeoutRef = useRef(0);
  useEffect(() => {
    if (promptTimeout) {
  ...
  const stopOnIdleRef = useRef(stopOnIdle);
  useEffect(() => {
    stopOnIdleRef.current = stopOnIdle;
  }, [stopOnIdle]);
  const immediateEventsRef = useRef(immediateEvents);
  const elementRef = useRef(element);
  const eventsRef = useRef([...new Set([...events, ...immediateEvents]).values()]);
  const emitOnPresenceChange = useRef(onPresenceChange);
  useEffect(() => {
  ...
  const emitOnMessage = useRef(onMessage);
  useEffect(() => {
    emitOnMessage.current = onMessage;
  }, [onMessage]);
  const callOnAction = useMemo(() => {
  ...
  const handleEvent = useRef(eventHandler);
  useEffect(() => {
  ...
  const start = useCallback((remote) => {
    destroyTimeout();
    bindEvents();
    idle.current = false;
    prompted.current = false;
  ...
  const reset = useCallback((remote) => {
    destroyTimeout();
    bindEvents();
  ...
  const activate = useCallback((remote) => {
    destroyTimeout();
    bindEvents();
    if (idle.current || prompted.current) {
      toggleActive();
  ...
  const resume = useCallback((remote = false) => {
    if (!paused.current)
      return false;
    paused.current = false;
    if (!prompted.current) {
  ...
  const message = useCallback((data, emitOnSelf) => {
    if (manager.current) {
      if (emitOnSelf)
  ...
  const isLastActiveTab = useCallback(() => {
    if (!manager.current)
      return null;
  ...
  const getLastIdleTime = useCallback(() => {
    if (!lastIdle.current)
      return null;
  ...
  const getTotalIdleTime = useCallback(() => {
    if (idle.current) {
...
var IdleTimerContext = createContext(null);
function IdleTimerProvider(props) {
  const idleTimer = useIdleTimer(props);
  return /* @__PURE__ */ React2.createElement(IdleTimerContext.Provider, {
    value: idleTimer
  }, props.children);
}
